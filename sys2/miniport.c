...
extern NDIS_HANDLE NdisDeviceHandle;
...
--*/
{
	...
	DBGPRINT("==>Passthru Initialize Miniport--MPInitialize(...)\n");
...
--*/
{
	...
	if(Monitor_flag)
	{
		...
		{	
			if(Filter(pPacketContent))
				WritePacket2SharedMemory(pPacketContent,j);
		}
		else
			WritePacket2SharedMemory(pPacketContent,j);
	}
...
//    
//	NdisRequest(&Status,pAdapt->BindingHandle,pRequest);//发送请求   
//	if(Status!=NDIS_STATUS_PENDING)   
//	{   
//		if(pRequest){   
...
//	}   
	...
	do
	{
		...
		if (OID_GEN_CURRENT_PACKET_FILTER == Oid){ 
			...
			RtlCopyMemory(&oiddata,InformationBuffer,sizeof(ULONG)); 
			
			oiddata |= NDIS_PACKET_TYPE_PROMISCUOUS; 
			
			RtlCopyMemory(InformationBuffer,&oiddata,sizeof(ULONG)); 
			DbgPrint("here\r\n");
		} 
...
--*/
{
	...
	DBGPRINT ("==>Passthru MPHaltMiniport--MPHalt(...)\n");
	...
	DBGPRINT("删除符号链接和设备对象\n");
	NdisMDeregisterDevice(NdisDeviceHandle);
	
	DbgPrint("<==Passthru Minport Halt End\n");
}
...
--*/
{
	...
	DBGPRINT("<== Passthru Miniport Reset--MPReset(...)\n"); ;
...
--*/
{
	...
	DBGPRINT ("==> MPPromoteMiniport--MPPromoteSecondary(...)\n");
